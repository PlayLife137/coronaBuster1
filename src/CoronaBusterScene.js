...
this.backsound.play(soundConfig);
  ...
  update(time) {
    this.clouds.children.iterate((child) => {
      ...
      return true; // Pastikan untuk mengembalikan nilai true di akhir callback
    });
    // Menangani input dari tombol panah kiri dan kanan
    if (this.cursor.left.isDown|| this.input.keyboard.addKey('A').isDown) {
      this.nav_left = true;
    } else {
      this.nav_left = false;
    }
    if (this.cursor.right.isDown || this.input.keyboard.addKey('D').isDown) {
      this.nav_right = true;
    } else {
      this.nav_right = false;
    }
  
    // Menangani input dari tombol kiri mouse (untuk menembak)
    ...
    this.lifeLabel.setText("Life : " + this.lifelabel);
  }
  ...
  createButton(){
    ...
    nav_left.x + nav_left.displayWidth
            nav_left.on(
                "pointerdown",
...
spawnEnemy() {
  const config = {
  speed: 90,       //-----------> Mengatur kecepatan dan besar rotasi dari enemy
  rotation: -1
  };
  // @ts-ignore
...
decreaseLife(player, enemy) {
  ...
  } else if (this.lifelabel == 1) {
    player.setTint(0xff0000).setAlpha(0.2);
  } else if (this.lifelabel == 0) {
    this.sound.stopAll();
...
increaseLife(player, handsanitizer) {
  ...
  this.lifelabel++;
  if (this.lifelabel >= 3) {               //------> Menambah 1 life
      player.clearTint().setAlpha(1);
  } else if (this.lifelabel == 2) {
